...
describe("authors can trust declared contracts", () => {
  it("CV-002: Text, Number, Boolean and List work without imports; Nothing stays an explicit return", () => {
  ...
  it("CV-003: an unavailable custom input type is an author error at its reference", () => {
  ...
  it("CV-004: declaring save does not declare the promised saveGame capability", () => {
  ...
  it("CV-004: a same-named type cannot satisfy a public capability promise", () => {
  ...
  it("CV-017: independent unavailable types retain exact original source positions", () => {
  ...
  it("CV-018: malformed source is rejected rather than repaired into a valid model", () => {
  ...
  it("CV-020: repeating a supplied input is deterministic and changed source is observed", () => {
  ...
  it.todo("CV-020: removing a supplied module invalidates an earlier accepted import", () => {
    const compiler = createCompiler();
  ...
  it("CV-022: a nearby unsupplied file neither declares a type nor authorizes project I/O", () => {
  ...
  it("CV-023: an always-successful implementation cannot satisfy the missing-capability expectation", () => {
  ...
  it("CV-023: an unimplemented compiler operation stays a failure", () => {
