...
import { Visiting } from '../support/visiting.js';

describe('An analysis author visits declared source', () => {
  it('SI-01: keeps capabilities, inputs and name locations in authored order', () => {
    const visiting = new Visiting();
    visiting.sourceIs('store.expec', `concept StoreGame {
  capability startup(configurations: SystemConfig)
  capability saveGame(snapshot: PlayerStateSnapshot)
}`);

    visiting.collectCapabilities();

    visiting.expectCapabilities([
  ...
  capability merge(left: List<Pair<Number>>, right: List<Pair<Number>>)
    ...
    visiting.collectTypeUses();

    visiting.expectTypeUses([
    ...
    visiting.expectDistinctTypeOccurrences(6);
  });
  ...
  capability saveGame(snapshot: PlayerStateSnapshot) {
    ...
    visiting.collectPromises();

    visiting.expectPromises([
  ...
  it('SI-06: preserves syntax rejection instead of manufacturing an inspectable description', () => {
    ...
    visiting.expectRejectedSource('unexpected-token', { line: 1, column: 20 });
  });

  it('keeps a quoted dot inside one decoded reference segment', () => {
    const visiting = new Visiting();
    visiting.sourceIs('store.expec', 'type Alias = `State.Snapshot`');
