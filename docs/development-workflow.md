...
The next construction task builds the compiler and validation around those rules. Its [Ready specification](../specifications/compiler/README.md) now establishes the source model, declaration resolution, diagnostics, and independently observable results. Parsing, resolution, source tracking, and other internal responsibilities can be organized while implementing that contract; the board does not need a separate task for each internal component. The user's deferral of general labeled relationship notation remains explicit.
...
The repository contains **11 user stories and 78 acceptance-scenario definitions: 34 earlier EX, 19 CORE-01 grammar GR, and 25 CORE-18 compiler CV scenarios**. The grammar scenarios expand to 28 cases; the compiler scenarios expand to 56 cases with 67 fixtures. All are unbound: they have no runner or real execution bindings and have not run. Specifications and the example manifest were authored by hand. No compiler, generation implementation, or self-hosting system exists yet.

The [build backlog](planning/build-backlog.json) contains **18 substantial construction tasks** across the current solution and all 11 stories. **CORE-01 — Define the .expec language grammar** and **CORE-18 — Build the compiler and validation** are **Ready**; the remaining **16 tasks are Captured**. All hypotheses are **Untested**. The earlier 62-task decomposition is retained separately as historical planning material.

The [CORE-01 specification](../specifications/grammar/README.md) and [CORE-18 specification](../specifications/compiler/README.md) are ready for implementation within their stated scope; relationships derive from existing declarations without a dedicated keyword. The [compiler acceptance cases](../specifications/acceptance/compiler.feature) define future observations of the real implementation. Follow the [construction sequence](build-plan.md) through configuration, composition, project integration, outputs, preservation, and self-development. The [specification guide](../specifications/README.md) retains the earlier EX examples and draft sketches as history, not the current API. Develop further examples and internal steps within each construction task as the work makes them concrete.
