...
Current status: **18 construction tasks, 11 user stories, and 78 unbound acceptance-scenario definitions: 34 earlier EX, 19 grammar GR, and 25 compiler CV scenarios.** CORE-01 grammar and CORE-18 compiler/validation are **Ready**; the remaining 16 tasks are **Captured**, and all hypotheses are **Untested**. Ready describes specified work, not implemented behavior. No compiler, working generation, or executed acceptance tests exist. `.expec` is the working name used by this workspace.
...
Current artifacts capture design; tasks backfilled from them do not establish implementation or a supported hypothesis. Acceptance scenarios are explicitly unbound: their future test helpers must invoke the real compiler and independently observe its results before any scenario can count as verified. Learning should revise the proposal when necessary, without hiding failed checks or treating passing examples as proof beyond their scope.
