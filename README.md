...
The original design artifacts remain distinct from implementation evidence. Bound acceptance tests invoke the real compiler and independently observe its results; the [coverage inventory](specifications/acceptance/coverage.json) keeps the remaining work visible. Learning should revise the proposal when necessary, without hiding failed checks or treating passing examples as proof beyond their scope.
