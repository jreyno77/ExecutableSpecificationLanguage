...
**Candidate 0.1 · September 25, 2026 · Specification Ready; task Implementing · Full-scope hypothesis Inconclusive.** Relationships derive from ordinary declarations. The candidate now has an ANTLR-based reader and scoped tests; [implementation evidence](../../docs/implementation-evidence.md) records their limits. The Ready specification is the baseline, and learning can still revise its notation and contracts.

**Review amendments:** `Text` is a primitive, and primitives require no imports. The agreed reader operation is `read`, with `ReadResult` as its working result name. Both `relates "owns" to Cart` and the later `owns cart: Cart` / `uses storage: Storage` style were rejected. Neither style is candidate syntax. GR-010 preserves declarations from which relationship views are derived; no relationship keyword is needed. The implementation uses antlr-ng with antlr4ng; versions and commands are pinned in the development setup. Shortening ideas remain design learning, not an adopted syntax redesign.
...
| [grammar.feature](../acceptance/grammar.feature) | GR-001–GR-019: 19 definitions, 28 expanded cases; GR-001 bound, remaining complete scenarios unbound. |
...
CORE-18 supplies the reader/model integration, diagnostics, and acceptance boundary, with an initial implementation and selected executable bindings. GR-010 uses existing declaration forms to preserve relationship direction and provenance. Source-only grammar acceptance does not require semantic validation. Configuration, composition, connected projects, target generation, execution drivers, and safe project updates retain their construction tasks. The [coverage inventory](../acceptance/coverage.json) identifies remaining complete GR bindings.
...
**Historical design learning:** the earlier sketches used overlapping notation for names, strings, and promises. This candidate separates them and distinguishes declaration availability from text recognition. That original design observation was not execution evidence. Current implementation evidence is recorded separately; author reviews should still challenge ambiguous or cumbersome forms without treating selected passing cases as proof of the entire grammar.
