...
Status: authored specification only. [source-model.expec](source-model.expec) makes the reader's previously opaque `SourceDescription` concrete for CORE-18 compiler/validation design. It does not implement a reader, validator, compiler, generated parser, or executable tests. The records and their field names are a proposed public contract. They are not ANTLR parse-tree classes or a prescribed in-memory class hierarchy.
...
The model is usable for independent acceptance assertions: a driver can find the public-reference node for `saveGame`, locate the distinct declaration/name nodes for `save`, and check that no declaration for `saveGame` was invented; follow a shopping check's call to `bookQuantity` and its equality operands; or inspect the ordered message nodes and captured reply. These describe future observations of a real result. They are not evidence that the fixtures have been read, compiled, or executed today.
