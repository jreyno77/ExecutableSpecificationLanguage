...
The current set has 18 construction tasks, 11 stories, and **78 scenario definitions: 34 earlier EX, 19 grammar GR, and 25 compiler CV scenarios**. All remain unbound and unexecuted. **CORE-01 — Define the .expec language grammar** and **CORE-18 — Build the compiler and validation** are **Ready**; the remaining 16 tasks are **Captured**, and all hypotheses are **Untested**. No compiler or generation implementation exists. Ready describes specified work, not completed software or a tested hypothesis.

## Ready specifications: grammar and compiler

[CORE-01's detailed specification](grammar/README.md) develops the first construction task into a syntax proposal, lexical rules, source fixtures, the `SyntaxReader.read` contract, and [GR-001–GR-019 acceptance scenarios](acceptance/grammar.feature), comprising 28 cases when outline rows are expanded. These cases specify authored meaning and useful diagnostics through a future real reader. They do not run the described software, load imports, or generate targets.

The candidate distinguishes names from strings, literal expectations from prose obligations, and declared capabilities from public references. Syntax reading can retain a `saveGame`/`save` mismatch; semantic validation must reject it during compilation. Review confirmed import-free primitives and the working names `read` and `ReadResult`. Both suggested relationship styles were rejected; the user then chose to derive relationships from existing declarations, with GR-010 revised accordingly. Dependencies and explicit ordered interactions remain specified. ANTLR is the preferred parser-generator candidate.
...
| [Grammar specification](grammar/README.md) | Define current language forms, source reading, supporting types, and source provenance. | Candidate 0.1, Ready; no parser or runtime implementation. |
| [Grammar acceptance cases](acceptance/grammar.feature) | GR-001–GR-019 with concrete source fixtures and authored observations. | Draft, unbound, and unexecuted. |
| [Compiler specification](compiler/README.md) | Define the compiler core, shared source model, validation rules, catalogs, and results. | Candidate 0.1, Ready; no implementation. |
| [Compiler acceptance cases](acceptance/compiler.feature) | CV-001–CV-025, 56 expanded cases, and 67 supporting fixtures. | Draft, unbound, and unexecuted. |
...
The `.feature` files use readable Given/When/Then notation to preserve the examples independently of the language grammar. This is a temporary acceptance-specification format, not a decision that .expec must embed Gherkin or depend on a particular BDD framework. The `@unbound` tag means the scenarios do not yet execute. It does not mean they have passed.
...
Test-runner choice and source-editing mechanisms remain open; CORE-18 now proposes compiler diagnostic and result schemas. A bare test name or unbound sentence is not an executable assertion. Bindings that throw not-implemented errors must fail or remain explicitly incomplete; they must not count as verified behavior.

## Bootstrap sequence

1. Use the Ready CORE-01 grammar specification and its valid/invalid examples, retaining the user's explicit deferral of labeled relationship notation.
2. Implement the Ready CORE-18 compiler/validation specification with an independent acceptance runner. Its contracts and examples define the observable work; internal implementation choices remain testable hypotheses.
...
Self-description, generating the compiler's structure, and full self-hosting are different milestones. The current contracts are authored by hand; no compiler or self-generation is implemented. The bootstrap provides the initial machinery, and independently authored examples provide behavioral checks. Passing selected examples does not verify the entire language. Counterevidence is a reason to learn and revise; if an expectation changes for a justified reason, retain that rationale and rerun the relevant checks instead of reusing evidence for the earlier requirement.
